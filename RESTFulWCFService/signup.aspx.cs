...
namespace RESTFulWCFService
{
    ...
    {  
        ...
        public string search_url()
        {
            ...
            Session["user_location_search"] = url;
            return url;
        ...
        private void registration()
        {
            try
            {
                ...
                int i = objbusinessmpartener.partener_signup(email, password, name, gender, dob, is_certified, phone, part_gen, part_age, postal, country_sk, state_sk, city_sk, area_sk, speciality_selection(), HttpContext.Current.Request.UserHostAddress.ToString());
                if (i != 0)
                {
                    lblmsg.Visible = true;
        ...
        protected void ddlcity_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlcountry.SelectedIndex > 0 && ddlstate.SelectedIndex > 0 && ddlcity.SelectedIndex > 0)
            {
                ...
                if (ddlcountry.SelectedIndex > 0)
                {
                    DataSet ds = objRegistrationBusiness.getAreaProvider(Convert.ToInt32(0), Convert.ToInt32(0), Convert.ToInt32(ddlcountry.SelectedValue));
                    if (ds != null && ds.Tables[0].Rows.Count > 0)
                    {
                        ddlArea.DataSource = ds.Tables[0];
                        ddlArea.DataTextField = "area_name";
