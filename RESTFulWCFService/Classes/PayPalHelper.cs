...
namespace PayPalIntegration
{
	...
	public class PayPalHelper
	{
		...
		public string GetSubmitUrl()
		{
            ...
            if (Currency_code != null && CancelUrl != "")
                url.AppendFormat("&currency_code={0}", HttpUtility.UrlEncode(Currency_code));

            if (landing_page != null && CancelUrl != "")
                url.AppendFormat("&landing_page={0}", HttpUtility.UrlEncode(landing_page));

			url.AppendFormat("&SHIPTOCOUNTRYCODE={0}", HttpUtility.UrlEncode("US"));
