...
namespace Business
{
    public class BusinessMPartener
    {
        ...
        public int partener_signup(string email_id, string password, string name, char gender, string dob, string is_certified, string phone, string desired_gender, int desired_age, string postal, int country_sk, int? state_sk, int? city_sk, int? area_sk, DataTable speciality, string ip_address)
        {
            int i;
            i = objfactorypartener.partener_signup(email_id, password, name, gender, dob, is_certified, phone, desired_gender, desired_age, postal, country_sk, state_sk, city_sk, area_sk, speciality, ip_address);
            return i;
        }
        ...
        public DataSet partner_login_oxpay(string Email_id)
        {
            DataSet ds = new DataSet();
            ds = objfactorypartener.partner_login_oxpay(Email_id);
            return ds;

        }
        public DataSet convert_to_sgd(string currency, string amount)
        {
            return objfactorypartener.convert_to_sgd(currency,amount);
        ...
        {
            DataSet ds = new DataSet();
            ds = objfactorypartener.Search_Partner(country_sk, city_sk, desired_age_group_sk, state_sk, area_sk, desired_gender, gender_sk, massage_type_sk, from_to, is_outcall);
            return ds;
        ...
        {
            DataSet ds = objfactorypartener.get_chat_users();
            return ds;
        }
        public DataSet get_QRs()
        {
            DataSet ds = objfactorypartener.get_QRs();
            return ds;
        }
        public DataSet get_user_country_payments(string country_sk)
        {
            return objfactorypartener.get_user_country_payments(country_sk);
        }

        public string getPymntImgs(string content)
        {
            return objfactorypartener.getPymntImgs(content);
        }
    }
}
